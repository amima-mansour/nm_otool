...
/*   Updated: 2019/08/30 11:03:46 by amansour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool manage_sections_32(t_file *file, void *lc)
{
	...
	sect = (struct section *)iscorrup(file, (void*)lc + sizeof(struct segment_command), sizeof(*sect));
	i = -1;
	while (++i < nsects)
	{
		if (!sect)
        ...
        //sect = (struct section *)iscorrup(file, (void*)sect + 1, sizeof(*sect));
		sect = sect + 1;
	}
	return (false);
}
...
bool manage_sections_64(t_file *file, void *lc)
{
	...
	sect = (struct section_64 *)iscorrup(file, (void*)lc + sizeof(struct segment_command_64), sizeof(*sect));
	i = -1;
	while (++i < nsects)
	{
		if (!sect)
